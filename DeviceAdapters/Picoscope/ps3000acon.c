	...
	int16_t nDigitalDirections)
{
	...
	{
		OutputDebugString("SetTrigger:ps3000aSetTriggerChannelProperties --- "+ status );
		return status;
	}

	if ((status = ps3000aSetTriggerChannelConditionsV2(unit->handle, triggerConditionsV2, nTriggerConditions)) != PICO_OK) 
	{
		OutputDebugString("SetTrigger:ps3000aSetTriggerChannelConditions ---"+ status);
		return status;
	}
	...
	{
		OutputDebugString("SetTrigger:ps3000aSetTriggerChannelDirections ---"+ status);
		return status;
	}

	if ((status = ps3000aSetTriggerDelay(unit->handle, delay)) != PICO_OK) 
	{
		OutputDebugString("SetTrigger:ps3000aSetTriggerDelay  ---"+ status);
		return status;
	}
	...
	if((status = ps3000aSetPulseWidthQualifierV2(unit->handle, 
		...
		pwq->type)) != PICO_OK)
	{
		OutputDebugString("SetTrigger:ps3000aSetPulseWidthQualifier ---"+ status);
		return status;
	}
	...
	if (unit->digitalPorts)					
	{
		...
		{
			OutputDebugString("SetTrigger:ps3000aSetTriggerDigitalPortProperties ---"+ status);
			return status;
		}
	}
	return status;
...
void collectRapidBlock(UNIT * unit)
{
	...
	//Run
	timebase = 160; // Sample interval will be device dependent
...
void picoInitBlock(UNIT * unit,long sampleOffset_)
{
	...
	int i =0;
	PICO_STATUS status;
	short triggerVoltage = mv_to_adc(1000, unit->channelSettings[PS3000A_CHANNEL_A].range, unit);  //触发电压1000mv

	struct tPS3000ATriggerChannelProperties sourceDetails = {	triggerVoltage,
		256 * 10,
		triggerVoltage,    //触发电压1000mv
		256 * 10,
		PS3000A_EXTERNAL,
		PS3000A_LEVEL};
	...
	unit->channelSettings[1].enabled  = 0;    //关闭B通道
	...
	setDefaults(unit);     //设置通道的属性

	/* Trigger enabled     开启外部触发
	* Rising edge
	* Threshold = 1000mV   sampleOffset_为触发延时用来设置采样偏移  */
...
void picoInitRapidBlock(UNIT * unit,long sampleOffset_,unsigned long timeout)
{
	...
	int i =0;
	PICO_STATUS status;
	short triggerVoltage = mv_to_adc(3000,5000, unit);    //触发电压3v
...
PICO_STATUS picoRunRapidBlock(UNIT * unit,unsigned short nCaptures,unsigned long nSamples,uint32_t *CompletedNSample,uint32_t *nCompletedCaptures,short * pBuf)
{
    int32_t timeIndisposed;
	short  channel;
	...
	if (status == PICO_OK)
	{
		...
		{ 		
			for (j=0; j<nSamples; j++)
			{
				...
				*(pBuf + lIndex) = *(pBuf + lIndex)+32768;   //+32768  防止像素点出现负值情况
			}
	...
	if (unit->handle) 
	{
		for (i = 0; i < 11; i++) 
		{
			...
			if (i == 3) 
			{
				...
				if (strlen((CONST char *)line) == 8 || line[3] != '4')
				{
					unit->ETS	= TRUE;
				}
				...
				// Check if MSO device
				if (strlen((CONST char *)line) >= 7)
				{
					line[4] = toupper(line[4]);
					line[5] = toupper(line[5]);
					line[6] = toupper(line[6]);

					if(strcmp((CONST char *)line + 4, "MSO") == 0 || strcmp((CONST char *)line + 5, "MSO") == 0 )
					{
						unit->digitalPorts = 2;
						unit->sigGen = SIGGEN_AWG;
					}
...
PICO_STATUS openDevice(UNIT *unit)
{
	...
	OutputDebugString("Handle: "+ unit->handle);

	if (status != PICO_OK) 
	{
		OutputDebugString("Unable to open device\n");
		OutputDebugString("Error code : "+ status);
		while(!_kbhit());
		exit(99); // exit program
	}

	OutputDebugString("Device opened successfully\n");

	// setup devices
	get_info(unit);
	timebase = 1;

	ps3000aMaximumValue(unit->handle, &value);    //32512
	unit->maxValue = value;

	for ( i = 0; i < unit->channelCount; i++) 
	{
		unit->channelSettings[i].enabled = TRUE;
		unit->channelSettings[i].DCcoupled = TRUE;      //直流耦合
		unit->channelSettings[i].range = PS3000A_5V;    //通道电压范围-5v~5v
	}
